...
int CHiVo::HiVo_start_wbc(VO_DEV source, VO_DEV destination, const VO_WBC_ATTR_S *pWbc_attr)
{
    HI_S32 ret_val = -1;

    if((ret_val = HI_MPI_VO_SetWbcAttr(source, pWbc_attr)) != 0)
    {
        DEBUG_ERROR( "CHiVo::HiVo_start_wbc HI_MPI_VO_SetWbcAttr FAILED(%d)(0x%08lx)\n", source, (unsigned long)ret_val);
        return -1;
    }

    if((ret_val = HI_MPI_VO_EnableWbc(source)) != 0)
    {
        DEBUG_ERROR( "CHiVo::HiVo_start_wbc HI_MPI_VO_EnableWbc FAILED(%d)(0x%08lx)\n", source, (unsigned long)ret_val);
        return -1;
    }
    //vo作为数据源发送回写(WBC)数据时，是以设备为发送者，向其他模块发送数据，用户将通道号置为0，SDK不检查输入的通道号
    MPP_CHN_S src_chn, dest_chn;

    src_chn.enModId    = HI_ID_VOU;
    src_chn.s32DevId   = source;
    src_chn.s32ChnId   = 0;

    dest_chn.enModId   = HI_ID_VOU;
#if defined(_AV_PLATFORM_HI3535_)||defined(_AV_PLATFORM_HI3520D_V300_)
    VO_LAYER vo_layer;
    if(HiVo_get_vo_layer(destination, vo_layer) != 0)
    {
        DEBUG_ERROR( "get vo layer failed(%d)!\n", destination);
        return -1;
    }
    dest_chn.s32DevId  = vo_layer;
#else
    dest_chn.s32DevId  = destination;
#endif
    dest_chn.s32ChnId  = 0;

    if((ret_val = HI_MPI_SYS_Bind(&src_chn, &dest_chn)) != 0)
    {
        DEBUG_ERROR( "CHiVo::HiVo_start_wbc HI_MPI_SYS_Bind FAILED(%d)(0x%08lx)\n", source, (unsigned long)ret_val);
        return -1;
    }
...
int CHiVo::HiVo_stop_wbc(VO_DEV source, VO_DEV destination)
{
    MPP_CHN_S src_chn, dest_chn;
    HI_S32 ret_val = -1;
    ...
    if((ret_val = HI_MPI_SYS_UnBind(&src_chn, &dest_chn)) != 0)
    {
        DEBUG_ERROR( "CHiVo::HiVo_stop_wbc HI_MPI_SYS_UnBind FAILED(%d)(0x%08lx)\n", source, (unsigned long)ret_val);
        return -1;
    }

    if((ret_val = HI_MPI_VO_DisableWbc(source)) != 0)
    {
        DEBUG_ERROR( "CHiVo::HiVo_stop_wbc HI_MPI_VO_DisableWbc FAILED(%d)(0x%08lx)\n", source, (unsigned long)ret_val);
        return -1;
    }
...
int CHiVo::HiVo_enable_pip(VO_DEV vo_dev, VO_VIDEO_LAYER_ATTR_S PipLayerAttr)
{
    ...
    ret = HI_MPI_VO_BindVideoLayer(vo_layer, vo_dev);
    if(ret != 0)
    {
        DEBUG_ERROR("HI_MPI_VO_BindVideoLayer err:0x%08x\n", ret);
        return -1;
    }

    ret = HI_MPI_VO_SetVideoLayerAttr(vo_layer, &PipLayerAttr);
    if(ret != 0)
    {
    ...
    ret = HI_MPI_VO_EnableVideoLayer(vo_layer);
    if(ret != 0)
    {
    ...
    ret = HI_MPI_VO_SetVideoLayerPriority(vo_layer, 0);
    if(ret != 0)
    {
        DEBUG_ERROR("HI_MPI_VO_SetVideoLayerPriority err:0x%08x\n", ret);
        return -1;
    }
#else
    ret = HI_MPI_VO_PipLayerBindDev(vo_dev);
    if(ret != 0)
    {
        //DEBUG_ERROR( "HI_MPI_VO_PipLayerBindDev err:0x%08x\n", ret);
        DEBUG_ERROR("HI_MPI_VO_PipLayerBindDev err:0x%08x\n", ret);
        return -1;
    }

    DEBUG_MESSAGE("HiVo_enable_pip PipLayerAttr(w %d, h %d)\n", PipLayerAttr.stImageSize.u32Width, PipLayerAttr.stImageSize.u32Height);

    ret = HI_MPI_VO_SetPipLayerAttr(&PipLayerAttr);
    if(ret != 0)
    {
        DEBUG_ERROR( "HI_MPI_VO_SetPipLayerAttr err:0x%08x\n", ret);
        return -1;
    }

    ret = HI_MPI_VO_EnablePipLayer();
    if(ret != 0)
    {
        DEBUG_ERROR( "HI_MPI_VO_EnablePipLayer err:0x%08x\n", ret);
        return -1;
    }
#endif
...
int CHiVo::HiVo_disable_pip(VO_DEV vo_dev)
{
    ...
    ret = HI_MPI_VO_DisableVideoLayer(vo_layer);
    if(ret != 0)
    {
        DEBUG_ERROR("HI_MPI_VO_EnableVideoLayer err:0x%08x\n", ret);
        return -1;
    ...
    ret = HI_MPI_VO_UnBindVideoLayer(vo_layer, vo_dev);
    if(ret != 0)
    {
        DEBUG_ERROR("HI_MPI_VO_BindVideoLayer err:0x%08x\n", ret);
        return -1;
...
#else
    ret = HI_MPI_VO_DisablePipLayer();
    if(ret != 0)
    {
        DEBUG_ERROR( "HI_MPI_VO_DisablePipLayer err:0x%08x\n", ret);
        return -1;
    }

    ret = HI_MPI_VO_PipLayerUnBindDev(vo_dev);
    if(ret != 0)
    {
        DEBUG_ERROR( "HI_MPI_VO_PipLayerUnBindDev err:0x%08x\n", ret);
        return -1;
    }
#endif
    return 0;
...
int CHiVo::HiVo_set_dispos(VO_DEV vo_dev, int vo_chn, POINT_S pipPositon)
{
    int ret = 0;
    ...
    ret = HI_MPI_VO_SetChnDispPos(vo_layer, vo_chn, &pipPositon);
    if (ret !=0)
    {
...
#else
    ret = HI_MPI_VO_SetChnDispPos(vo_dev, vo_chn, &pipPositon);
    if (ret !=0)
    {
        DEBUG_ERROR( "HI_MPI_VO_SetChnDispPos err:0x%08x\n", ret);
        return -1;
...
int CHiVo::HiVo_refresh_vo(VO_DEV vo_dev, int vo_chn)
{
    ...
    ret = HI_MPI_VO_RefreshChn(vo_layer, vo_chn);
#else    
