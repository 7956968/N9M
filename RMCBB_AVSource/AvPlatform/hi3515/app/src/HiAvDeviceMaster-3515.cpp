...
int CHiAvDeviceMaster::Ae_bind_vdec_to_vo(int vdecchn, int vodev,int vochn, bool bind_flag)
{
	HI_S32 retval = 0;;
	if(bind_flag)
	{
		retval = HI_MPI_VDEC_BindOutput(vdecchn,vodev,vochn);
		if(retval != HI_SUCCESS)
		{
			DEBUG_ERROR("HI_MPI_VDEC_BindOutput FAILED(0x%08lx) vdecchn:%d vodev:%d vochn:%d\n",retval,vdecchn,vodev,vochn);
		}
	}
	else
	{
		retval = HI_MPI_VDEC_UnbindOutput(vdecchn);
		if(retval != HI_SUCCESS)
		{
			DEBUG_ERROR("HI_MPI_VDEC_UnbindOutput FAILED(0x%08lx) vdecchn:%d vodev:%d vochn:%d\n",retval,vdecchn,vodev,vochn);
		}
	}
	return 0;
}

int CHiAvDeviceMaster::Ae_bind_vi_to_vo(int videv,int vichn,int vodev, int vochn, bool bind_flag)
{
	HI_S32 retval = 0;;
	if(bind_flag)
	{
		retval = HI_MPI_VI_BindOutput(videv,vichn,vodev,vochn);
		if(retval != HI_SUCCESS)
		{
			DEBUG_ERROR("HI_MPI_VDEC_BindOutput FAILED(0x%08lx) vichn:%d vodev:%d vochn:%d\n",retval,vichn,vodev,vochn);
		}
	}
	else
	{
		retval = HI_MPI_VI_UnBindOutput(videv,vichn,vodev,vochn);
		if(retval != HI_SUCCESS)
		{
			DEBUG_ERROR("HI_MPI_VDEC_UnbindOutput FAILED(0x%08lx) vichn:%d vodev:%d vochn:%d\n",retval,vichn,vodev,vochn);
		}
	}
	return 0;
}

int CHiAvDeviceMaster::Ad_control_3515_vo(av_output_type_t output, int index, vpss_purpose_t vpss_purpose, int phy_video_chn_num, int vo_chn, int control_flag)
{	
	int videv = 0;
	int vichn = 0;
	HI_S32 ret_val = 0;
	if ((phy_video_chn_num >= pgAvDeviceInfo->Adi_max_channel_number()) || (phy_video_chn_num < 0))
	{
		DEBUG_ERROR( "CHiAvDeviceMaster::Ad_control_3515_vo (output:%d) (index:%d)(vpss_purpose:%d)(layout_chn:%d)(phy_chn %d), control_flag %d is illegal\n", output, index, vpss_purpose, vo_chn, phy_video_chn_num, control_flag);
		return -1;
	}
	...
	if(0 == control_flag)
	{
		...
		if((_VP_PLAYBACK_VDEC_ == vpss_purpose) || (_VP_PREVIEW_VDEC_ == vpss_purpose))
		{
			/*绑定VDEC 到VO*/
			Ae_bind_vdec_to_vo(phy_video_chn_num,m_pHi_vo->HiVo_get_vo_dev(output,1,0),vo_chn,1);
			av_split_t split_mode = m_vo_chn_state[output][index].cur_split_mode;
			av_vag_hdmi_resolution_t resolution = Ad_get_resolution(output, index);
			av_rect_t video_rect;
			av_rect_t vo_chn_rect;
			...
			if(HI_SUCCESS != (ret_val = HI_MPI_VO_GetChnAttr(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), vo_chn, &vo_chn_attr)))
			{
				DEBUG_ERROR( "CHiAvDeviceMaster::Ad_control_3515_vo HI_MPI_VO_GetChnAttr FAILED(0x%08lx)\n", (unsigned long)ret_val);
				return -1;
			}
			if(((int)vo_chn_attr.stRect.s32X != vo_chn_rect.x) || ((int)vo_chn_attr.stRect.s32Y != vo_chn_rect.y) || ((int)vo_chn_attr.stRect.u32Width != vo_chn_rect.w) || ((int)vo_chn_attr.stRect.u32Height != vo_chn_rect.h))
			{
				DEBUG_ERROR("vodev:%d vo_chn:%d src: x=%d y=%d w=%d h=%d dst: x=%d y=%d w=%d h=%d\n",m_pHi_vo->HiVo_get_vo_dev(output, index, 0),vo_chn,vo_chn_attr.stRect.s32X,vo_chn_attr.stRect.s32Y,vo_chn_attr.stRect.u32Width,vo_chn_attr.stRect.u32Height,vo_chn_rect.x,vo_chn_rect.y,vo_chn_rect.w,vo_chn_rect.h);	   

				vo_chn_attr.stRect.s32X = vo_chn_rect.x;
				vo_chn_attr.stRect.s32Y = vo_chn_rect.y;
				vo_chn_attr.stRect.u32Width = vo_chn_rect.w;
				vo_chn_attr.stRect.u32Height = 576;//vo_chn_rect.h;
				vo_chn_attr.bDeflicker = HI_TRUE;
				HI_MPI_VO_DisableChn(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), vo_chn);
				if(HI_SUCCESS != (ret_val = HI_MPI_VO_SetChnAttr(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), vo_chn, &vo_chn_attr)))
				{
					DEBUG_ERROR( "CHiAvDeviceMaster::Ad_control_3515_vo HI_MPI_VO_SetChnAttr FAILED(0x%08lx)\n", (unsigned long)ret_val);
					return -1;
				}
			}
			HI_MPI_VO_EnableChn(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), vo_chn);
		...
		{
			/*绑定VI 到VO*/
			Ae_bind_vi_to_vo(videv,vichn,m_pHi_vo->HiVo_get_vo_dev(output,1,0),vo_chn,1);
		}
	}
	else
	{
		...
		{
			/*去除绑定VDEC 到VO*/
			Ae_bind_vdec_to_vo(phy_video_chn_num,m_pHi_vo->HiVo_get_vo_dev(output,1,0),vo_chn,0);
		}
		else
		{
			/*去除绑定VI 到VO*/
			Ae_bind_vi_to_vo(videv,vichn,m_pHi_vo->HiVo_get_vo_dev(output,1,0),vo_chn,0);
		}
...
#else
        vo_pub_attr.enIntfType = VO_INTF_VGA;/*vga*/
#endif
        vo_pub_attr.enIntfSync = m_pHi_vo->HiVo_get_vo_interface_sync(resolution);

#if !defined(_AV_PLATFORM_HI3535_)&&!defined(_AV_PLATFORM_HI3520D_V300_)&&!defined(_AV_PLATFORM_HI3515_)
...
int CHiAvDeviceMaster::Ad_start_video_layer_V1(av_output_type_t output, int index, av_tvsystem_t tv_system, av_vag_hdmi_resolution_t resolution)
{
    ...
    display_frm_rt = 30;
#endif
    DEBUG_ERROR("cxliu,display_frm_rt=%d,resolution=%d\n",display_frm_rt,resolution);
	...
	else if(output == _AOT_CVBS_)
	{
        ...
        video_layer_attr.stDispRect.s32X = 0;//(720 -  pgAvDeviceInfo->Adi_get_D1_width()) / 2;
        video_layer_attr.stDispRect.s32Y = 0;
        m_pHi_vo->HiVo_get_video_size(tv_system, (int *)&video_layer_attr.stImageSize.u32Width, (int *)&video_layer_attr.stImageSize.u32Height, NULL, NULL);
        video_layer_attr.stDispRect.u32Width = video_layer_attr.stImageSize.u32Width;
        video_layer_attr.stDispRect.u32Height = video_layer_attr.stImageSize.u32Height;
		video_layer_attr.stImageSize.u32Width = video_layer_attr.stImageSize.u32Width;
		video_layer_attr.stImageSize.u32Height = video_layer_attr.stImageSize.u32Height;
        video_layer_attr.u32DispFrmRt = (tv_system == _AT_PAL_)?25:30;
		video_layer_attr.s32PiPChn = -1;
		printf("hswang w:%d h:%d\n",video_layer_attr.stDispRect.u32Width,video_layer_attr.stDispRect.u32Height);
        video_layer_attr.enPixFormat = pgAvDeviceInfo->Adi_get_video_layer_pixel_format();
        if(m_pHi_vo->HiVo_start_vo_video_layer(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), &video_layer_attr) != 0)
        {
            DEBUG_ERROR( "CHiAvDeviceMaster::Ad_start_video_layer_V1 m_pHi_vo->HiVo_start_vo_video_layer FAILED\n");
...
int CHiAvDeviceMaster::Ad_create_vo_layout(VO_DEV vo_dev, av_rect_t *pVideo_rect, av_split_t split_mode)
{
    ...
    for(vo_chn = 0; vo_chn < max_vo_chn_num; vo_chn ++)
    {
        ...
        vo_chn_attr.stRect.u32Width = vo_chn_rect.w;
        vo_chn_attr.stRect.u32Height = vo_chn_rect.h;
	    vo_chn_attr.bDeflicker = HI_TRUE; //使能通道抗闪烁,add by cxliu20151030

	    DEBUG_MESSAGE("\nAd_create_vo_layout,vo_dev=%d,vo_chn=%d\n",vo_dev,vo_chn);
...
int CHiAvDeviceMaster::Ad_create_selected_vo_layout(VO_DEV vo_dev, av_rect_t *pVideo_rect, av_split_t split_mode, int vo_chn, int vo_index)
{
    ...
    vo_chn_attr.bDeflicker = HI_TRUE; 
    DEBUG_ERROR(" create selected vo deflick!\n");
    if(m_pHi_vo->HiVo_start_vo_chn(vo_dev, vo_chn, &vo_chn_attr) != 0)
    {
        DEBUG_ERROR( "CHiAvDeviceMaster::Ad_create_vo_layout m_pHi_vo->HiVo_start_vo_chn %d FAILED\n", vo_chn);
        return -1;
...
#if defined(_AV_PLATFORM_HI3515_)
HI_S32 CHiAvDeviceMaster::Hi3515_ViBindVo(HI_S32 s32ViChnTotal, VO_DEV VoDev)
{
    HI_S32 ViChnCnt;
...
//        for (ViChn = 0; ViChn < s32ViChnPerDev; ViChn++)
        {
            s32Ret = HI_MPI_VI_BindOutput(ViDev, ViChn, VoDev, VoChn);
                printf("bind vi2vo , vi(%d,%d),vo(%d,%d)!\n",
...
int CHiAvDeviceMaster::Ad_start_video_output_V1(av_output_type_t output, int index, int type, av_split_t split_mode, int chn_layout[_AV_SPLIT_MAX_CHN_], int margin[4]/* = NULL*/)
{
...
#endif
#if defined(_AV_PLATFORM_HI3515_) && defined(_AV_HISILICON_VPSS_)
        vpss_purpose_t vpss_purpose;
        Ad_get_vpss_purpose(output, index, type, chn_layout[chn_index], vpss_purpose, false);
	DEBUG_ERROR("\nxxcxliu,output[%d],index[%d],type[%d],vpss_purpose[%d],chn_layout[chn_index](%d),chn_index[%d]\n",
	output,index,type,vpss_purpose,chn_layout[chn_index],chn_index);
	
        if ((vpss_purpose == _VP_SPOT_VDEC_)
            || (vpss_purpose == _VP_PREVIEW_VI_)
            || (vpss_purpose == _VP_SPOT_VI_))
        {
            Ad_control_3515_vo(output, index, vpss_purpose, chn_layout[chn_index], chn_index, 0);
        }
#endif
    }
	...
	if(i>=4)
	{
        ...
        int s32Ret = HI_MPI_VO_EnableChn(VoDev, j);
        DEBUG_MESSAGE("Enable VoDev %d ,vo channel %d failed with ec %#x\n", VoDev,j, s32Ret);

        if (s32Ret)
        {
            printf("Enable VoDev %d ,vo channel %d failed with ec %#x\n", VoDev,i, s32Ret);
        }
    }
    ...
    for(chn_index = 0; chn_index < max_vo_chn_num && chn_index < maxChlNum; chn_index ++)
    {
        ...
        if(0 != ((chn_mask) & (1ll<<chn_layout[chn_index])))
        {
            ...
            if ((vpss_purpose == _VP_SPOT_VDEC_)
                ...
                || (vpss_purpose == _VP_SPOT_VI_))
            {
                Ad_control_3515_vo(output, index, vpss_purpose, chn_layout[chn_index], VoChn, 0);
            }
#endif
            continue;
        }
        ...
        || (_VP_PLAYBACK_VDEC_ == vpss_purpose))
    {
	  ...
	  //  DEBUG_ERROR( "CHiAvDeviceMaster::Ad_output_source_control_V1 (output:%d) (index:%d)(vpss_purpose:%d)(layout_chn:%d)(phy_chn %d)\n", output, index, vpss_purpose, layout_chn, phy_chn);
		...
		if ((_VP_PREVIEW_VI_ == vpss_purpose) || (_VP_SPOT_VI_ == vpss_purpose) 
			...
			|| (_VP_PLAYBACK_VDEC_ == vpss_purpose))
		{
			return Ad_control_3515_vo(output, index, vpss_purpose, phy_chn, layout_chn, control_flag);
		}
#endif
    ...
    for(chn_index = 0; chn_index < max_vo_chn_num && chn_index<max_video_chn_num; chn_index ++)
    {
        ...
        if((vpss_purpose == _VP_PREVIEW_VI_) || (vpss_purpose == _VP_SPOT_VI_))/*preview*/
        {
			...
			if ((_VP_PREVIEW_VI_ == vpss_purpose) || (_VP_SPOT_VI_ == vpss_purpose)
				...
				|| (_VP_PLAYBACK_VDEC_ == vpss_purpose))
			{
				Ad_control_3515_vo(output, index, vpss_purpose, chn_layout[chn_index], chn_index, 1);
			}
...
_SWITCH_DECODER_:
    ...
    for(chn_index = 0; chn_index < pgAvDeviceInfo->Adi_get_split_chn_info(split_mode); chn_index ++)
    {
        ...
        vo_chn_attr.stRect.u32Width = vo_chn_rect.w;
        vo_chn_attr.stRect.u32Height = vo_chn_rect.h;
        vo_chn_attr.bDeflicker = HI_TRUE; //! for 3515 /13
...
#if defined(_AV_HAVE_VIDEO_DECODER_)&&defined(_AV_HISILICON_VPSS_)
...
#elif defined(_AV_PLATFORM_HI3518C_) || defined(_AV_PLATFORM_HI3516A_)||defined(_AV_PLATFORM_HI3515_)
#else
    #error "You must give the implement"
#endif
...
#if !defined(_AV_PRODUCT_CLASS_IPC_)
int CHiAvDeviceMaster::Ad_set_video_format( int chn, unsigned int chn_mask,unsigned char av_video_format)
{
    DEBUG_WARNING( "CHiAvDevice::Ad_set_video_format channel=%d av_video_format=%d\n", chn, av_video_format);
    int ret = -1;

    ret = m_pHi_vi->HiVi_set_ahd_video_format( chn, chn_mask,av_video_format);

    return ret;
}

int CHiAvDeviceMaster::Ad_is_ahd_video_format_changed()
{
    DEBUG_CRITICAL( "CHiAvDevice::Ad_is_ahd_video_format_changed\n");
    int ret = -1;

    ret = m_pHi_vi->HiVi_is_ahd_video_format_changed();
...
int CHiAvDeviceMaster::Ad_start_preview_dec(av_output_type_t output, int index, av_split_t split_mode, int chn_layout[_AV_SPLIT_MAX_CHN_])
{
    ...
    for (int index = 0; index < maxchnnum; index++)
    {
	 ...
	 HI_S32 PicSize=0;
        ...
        VB_PIC_BLK_SIZE(1280, 720, PT_H264, PicSize);	
...
int CHiAvDeviceMaster::Ad_switch_audio_chn(int type, unsigned int audio_chn)
{
    if (type == 0) // preview
    {
        ...
        if( !pgAvDeviceInfo->Adi_get_audio_cvbs_ctrl_switch() ) //! according to the preview param in general 
        {
            return 0;
        }
        ...
        {
            printf("reset ao bind!\n"); 
            Ad_unbind_preview_ao();
            Ad_bind_preview_ao(audio_chn);
        }
...
#if defined(_AV_HAVE_AUDIO_INPUT_) && defined(_AV_HAVE_AUDIO_OUTPUT_)
#if defined(_AV_PLATFORM_HI3515_)
int CHiAvDeviceMaster::Ad_bind_preview_ao(int audio_chn)
{
    m_pHi_ao->HiAo_start_ao(_AO_PLAYBACK_CVBS_);
    AUDIO_DEV ai_dev;
    AI_CHN ai_chn;
    m_pHi_ai->HiAi_get_ai_info(_AI_NORMAL_, audio_chn, &ai_dev, &ai_chn, NULL);
    
    ai_chn = m_preview_audio_ctrl.phy_chn; //! 0425
    int ret_val = -1;
    if((ret_val = m_pHi_ao->HiAo_ai_bind_ao(_AO_PLAYBACK_CVBS_, ai_dev, ai_chn)) != 0)
    {
        DEBUG_ERROR("HiAo_ai_bind_ao failed!(0x%x)\n", ret_val);
        return -1;
    }

    return 0;
}
int CHiAvDeviceMaster::Ad_unbind_preview_ao( )
{
    AUDIO_DEV ai_dev;
    AI_CHN ai_chn;
    if ((m_preview_audio_ctrl.phy_chn < 0 )|| (  m_preview_audio_ctrl.phy_chn > MAX_AUDIO_CHNNUM))
    {
        return -1;
    }
    m_pHi_ai->HiAi_get_ai_info(_AI_NORMAL_, m_preview_audio_ctrl.phy_chn, &ai_dev, &ai_chn, NULL);
    
    ai_chn = m_preview_audio_ctrl.phy_chn; //! 0425

    int ret_val = -1;
    if((ret_val = m_pHi_ao->HiAo_ai_unbind_ao(_AO_PLAYBACK_CVBS_, ai_dev, ai_chn)) != 0)
    {
        DEBUG_ERROR("HiAo_ai_unbind_ao failed!(0x%08x)\n", ret_val);
        return -1;
    }

    m_pHi_ao->HiAo_stop_ao(_AO_PLAYBACK_CVBS_);
...
{
	prctl(PR_SET_NAME, "Ad_preview_audio");
...
int CHiAvDeviceMaster::Ad_preview_audio_ctrl(bool start_or_stop)
{
    ...
    else
    {
        m_preview_audio_ctrl.bAudioPauseState = false;
        m_preview_audio_ctrl.audio_start = start_or_stop;
        while( m_preview_audio_ctrl.bAudioPauseState == false )
        {
...
// #else
            ...
            if (m_pHi_ai->HiAi_release_ai_frame(ai_type, phy_audio_chn, &audio_frame) != 0)
            {
...
int CHiAvDeviceMaster::Ad_UpdateDevMap( char szChannelMap[SUPPORT_MAX_VIDEO_CHANNEL_NUM] )
{
...
#if defined(_AV_HAVE_VIDEO_ENCODER_)&&defined(_AV_HISILICON_VPSS_)
	...
	if( !m_pSnapTask && m_pHi_vi && m_pAv_encoder)
    {
        m_pSnapTask = new CAvSnapTask( m_pHi_vi, (CHiAvEncoder*)m_pAv_encoder);
...
{
	DEBUG_CRITICAL( "#########CHiAvDeviceMaster::Ad_start_snap\n");
    if( !m_pSnapTask )
    {
#if defined(_AV_HISILICON_VPSS_)
        m_pSnapTask = new CAvSnapTask( m_pHi_vi, (CHiAvEncoder*)m_pAv_encoder, m_pHi_vpss );
...
int CHiAvDeviceMaster::Ad_cmd_signal_snap_picutres( AvSnapPara_t &pstuSignalSnapParam )
{
    ...
    DEBUG_CRITICAL("Ad_cmd_signal_snap_picutres\n");
    ...
    for(chn_index = 0; chn_index < max_vo_chn_num && chn_index < max_video_chn_num; chn_index++)
    {
        if(m_vo_chn_state[output][index].chn_layout[chn_index] == chn)
        {
			...
			if((vpss_purpose == _VP_PREVIEW_VDEC_) || (vpss_purpose == _VP_SPOT_VDEC_) || (vpss_purpose == _VP_PREVIEW_VI_)\
				|| (vpss_purpose == _VP_SPOT_VI_) || (vpss_purpose == _VP_PLAYBACK_VDEC_))
			{
				ret_val = Ad_control_3515_vo(output, index, vpss_purpose, chn, chn_index, 1);			
			}
			m_pHi_vo->HiVo_clear_vo(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), chn_index);
			Ad_show_access_logo(m_pHi_vo->HiVo_get_vo_dev(output, index, 0), chn_index, true);
#endif
    ...
    for(chn_index = 0; chn_index < max_vo_chn_num && chn_index < max_video_chn_num; chn_index++)
    {
        if(m_vo_chn_state[output][index].chn_layout[chn_index] == chn)
        {
			...
			if((vpss_purpose == _VP_PREVIEW_VDEC_) || (vpss_purpose == _VP_SPOT_VDEC_) || (vpss_purpose == _VP_PREVIEW_VI_)\
				|| (vpss_purpose == _VP_SPOT_VI_) || (vpss_purpose == _VP_PLAYBACK_VDEC_))
			{
				ret_val = Ad_control_3515_vo(output, index, vpss_purpose, chn, chn_index, 0);
			}
#endif			
...
{
    DEBUG_CRITICAL("CHiAvDeviceMaster::Ad_add_report_station_task\n");
...
int CHiAvDeviceMaster::Ad_get_report_station_result()
{
    ...
    else
    {
        ...
        {
            DEBUG_MESSAGE("-----PLAY FINISH------\n");
            result = AUDIO_PLAY_FINISH;
        }
        else
        {
            DEBUG_MESSAGE("-----NO PLAY------\n");
