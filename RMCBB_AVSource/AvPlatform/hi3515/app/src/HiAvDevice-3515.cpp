...
#if defined(_AV_PRODUCT_CLASS_IPC_) && defined(_AV_PLATFORM_HI3516A_) && defined(_AV_SUPPORT_IVE_)   
    m_pHiIve = new CHiIve(_DATA_SRC_VPSS, _DATA_TYPE_YUV_422SP, (void *)m_pHi_vpss,  (CHiAvEncoder *)m_pAv_encoder);
#endif
...
int CHiAvDevice::Ad_start_vi(av_tvsystem_t tv_system, vi_config_set_t *pSource/* = NULL*/, vi_config_set_t *pPrimary_attr/* = NULL*/, vi_config_set_t *pMinor_attr/* = NULL*/)
{
    ...
    VIDEO_NORM_E viNorm = (tv_system==_AT_PAL_)?VIDEO_ENCODING_MODE_PAL:VIDEO_ENCODING_MODE_NTSC;
    m_pHi_vi->Hivi_set_vi_norm(viNorm);
...
#if (!defined(_AV_PLATFORM_HI3518C_)) && (!defined(_AV_PLATFORM_HI3516A_))//3518 sdk don't support this function
...
int CHiAvDevice::Ad_start_ive()
{
    if(NULL != m_pHiIve)
   {
        return m_pHiIve->ive_start_work();
   }
...
int CHiAvDevice::Ad_start_ive_face_detection()
{
    return m_pHiIve->ive_fd_start();
}

int CHiAvDevice::Ad_stop_ive_face_detection()
{
    return m_pHiIve->ive_fd_stop();
}
#endif
...
{
    m_pHiIve->ive_register_result_notify_callback(notify_func);
    return 0;
}
int CHiAvDevice::Ad_unregister_ive_detection_result_notify_callback()
{
    m_pHiIve->ive_unregister_result_notify_callback();
    return 0;
}
#endif
