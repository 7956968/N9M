...
HI_S32 AencAACCheckConfig(AACENC_CONFIG *pconfig)
{
    ...
    else if(pconfig->coderFormat == EAACPLUS)
    {
        ...
        if(pconfig->sampleRate == 32000)
        {
        	if(pconfig->bitRate < 12000 ||  pconfig->bitRate >= 18000)
        	{
        	    printf("EAACPLUS 32000 Hz bitRate(%d) should  be 12000 ~ 17999\n",pconfig->bitRate);
        	    return HI_ERR_AENC_ILLEGAL_PARAM;
        	}
        }
        else if(pconfig->sampleRate == 44100)
        {
        	if(pconfig->bitRate < 18000 ||  pconfig->bitRate >= 40000)
        	{
        	    printf("EAACPLUS 44100 Hz bitRate(%d) should  be 18000 ~ 39999\n",pconfig->bitRate);
        	    return HI_ERR_AENC_ILLEGAL_PARAM;
        	}
        }
        else if(pconfig->sampleRate == 48000)
        {
        	if(pconfig->bitRate < 40000 ||  pconfig->bitRate > 56000)
        	{
        	    printf("EAACPLUS 48000 Hz bitRate(%d) should  be 40000 ~ 56000\n",pconfig->bitRate);
        	    return HI_ERR_AENC_ILLEGAL_PARAM;
        	}
        }
        else
