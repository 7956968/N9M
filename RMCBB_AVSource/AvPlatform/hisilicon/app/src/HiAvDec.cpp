...
int CHiAvDec::Ade_operate_decoder_for_switch_channel(int purpose)
{
    ...
    if (VDP_PLAYBACK_VDEC == purpose)
    {
        ...
        for(int i = 0; i < chnnum; i++)  // i is only index
        {
...
int CHiAvDec::Ade_create_vdec_handle(int purpose, int chn, RMFI2_VIDEOINFO *pVideoInfo)
{
    ...
    if (Ade_get_avdev_info(purpose, chn, &pAvDecInfo) < 0) // chn is only a index of decinfo
    {
        return -1;
    }
    // pAvDecInfo.VdecChn is no longer used as the state of the status quo, but the channel to vdec
    /*
    VDEC_CHN vdec_chn = pAvDecInfo->VdecChn;
    if (vdec_chn < 0 || vdec_chn > VDEC_MAX_CHN_NUM)
    {
        return -1;
    }
    VDEC_CHN_STAT_S vdec_stat;
    HI_S32 ret = HI_MPI_VDEC_Query(vdec_chn, &vdec_stat);

    if ( (int)HI_ERR_VDEC_UNEXIST == ret)
    {
        pAvDecInfo->bCreateDecChl = false;
    }
*/
...
#ifdef _AVDEC_ADJUST_JUMP_
    if ((pAvDecInfo->RealPlayFrameRate != pVideoInfo->lFPS) && (VDP_PLAYBACK_VDEC == purpose))
    {
        m_ResetFrameRate[purpose] = true;
        pAvDecInfo->RealPlayFrameRate = pVideoInfo->lFPS;
    }
...
int CHiAvDec::Ade_create_vdec(int VdecChn, int w, int h)
{
    ...
    if(ret != 0)
    {
        if ((int)0xa0058004 == ret) // already created
        {
            return 1;
        }
...
int CHiAvDec::Ade_send_data_to_vdecEx(int purpose, int chn, unsigned char *pBuffer, int DataLen, const PlaybackFrameinfo_t* pstuFrameInfo)
{
    ...
    if(retval != 0)
    {
        ...
        if ((HI_U32)retval == 0xa0058005)  //!< vdec not created
        {
            return 1; //!< will release current frame and continue to next I frame  20160616
        }
        return -1;
    }
        ...
        case APT_G726:
            {
                ...
                {       
                    G726Attr.enG726bps = G726_16K;//MEDIA_G726_16K;//MEDIA_G726_40K ;
                }
...
int CHiAvDec::Ade_send_data_to_adec(int achn, unsigned char *pBuffer, int DataLen)
{
    ...
    retval = HI_MPI_ADEC_SendStream(achn, &AudioStream, HI_IO_NOBLOCK);   
    if(retval != 0)
    {
