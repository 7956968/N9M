...
#if !defined __AVIRIFF_H__
#define __AVIRIFF_H__
...
typedef char                 BYTE, TCHAR, *LPBYTE;
...
*/
/*
inline DWORD FCC(char *ch4)
{
   DWORD fcc;

   fcc = (ch4[0] | (ch4[1]<<8) | (ch4[2]<<16) | (ch4[3]<<24));
...
typedef struct _riffchunk {
   FOURCC fcc;
   DWORD  cb;
}__attribute__((packed)) RIFFCHUNK, * LPRIFFCHUNK;
typedef struct _rifflist {
   FOURCC fcc;
   DWORD  cb;
   FOURCC fccListType;
}__attribute__((packed)) RIFFLIST, * LPRIFFLIST;
...
}__attribute__((packed)) AVIEXTHEADER;
...
typedef struct _avimetaindex {
   ...
   DWORD  dwReserved[3];
   DWORD  adwIndex[];
}__attribute__((packed)) AVIMETAINDEX;
...
}__attribute__((packed)) AVISTDINDEX_ENTRY;
...
}__attribute__((packed)) AVITIMEDINDEX_ENTRY;
...
}__attribute__((packed)) AVITIMEDINDEX;
...
}__attribute__((packed)) AVITIMECODEINDEX;
...
typedef struct _avitcdlindex_entry {
    ...
    DWORD    dwUser;
    TCHAR    szReelId[12];
}__attribute__((packed)) AVITCDLINDEX_ENTRY;
...
}__attribute__((packed)) AVITCDLINDEX;
...
}__attribute__((packed)) AVIFIELDINDEX, * PAVIFIELDINDEX;
