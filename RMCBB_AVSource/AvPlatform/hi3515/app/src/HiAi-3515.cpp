    ...
    if (pAi_dev != NULL || pAi_chn != NULL)
    {
        switch (ai_type)
        {
            case _AI_NORMAL_:
            {
               ...
               _AV_POINTER_ASSIGNMENT_(pAi_dev, 1); //! 1 origin
               _AV_POINTER_ASSIGNMENT_(pAi_chn, phy_chn);		
 #else
                _AV_POINTER_ASSIGNMENT_(pAi_dev, 0);
                _AV_POINTER_ASSIGNMENT_(pAi_chn, phy_chn);
            ...
            case _AI_TALKBACK_:
            {
                ...
                _AV_POINTER_ASSIGNMENT_(pAi_chn, 0);
#elif defined(_AV_PLATFORM_HI3520D_)||defined(_AV_PLATFORM_HI3515_)
                _AV_POINTER_ASSIGNMENT_(pAi_dev, 0);
                _AV_POINTER_ASSIGNMENT_(pAi_chn, 0); //! previous 8
#elif defined(_AV_PLATFORM_HI3520D_V300_)
				if(product_type == PTD_6A_II_AVX)
				{
	               _AV_POINTER_ASSIGNMENT_(pAi_dev, 0);
    ...
    if (pAi_attr != NULL)
    {
...
#else
        pAi_attr->enSoundmode = AUDIO_SOUND_MODE_MOMO;
		//pAi_attr->u32ClkSel = 0;
		//pAi_attr->u32EXFlag = 1;
#ifdef _AV_USE_AACENC_
		pAi_attr->enSamplerate = AUDIO_SAMPLE_RATE_16000;
        pAi_attr->u32PtNumPerFrm = 1024;
#endif
#endif

#if defined(_AV_PLATFORM_HI3518C_)
        pAi_attr->enWorkmode = AIO_MODE_I2S_MASTER;
#elif defined(_AV_PLATFORM_HI3516A_)
        ...
        switch (ai_type)
        {
            ...
            audio_chn_num = 16;
                ...
                audio_chn_num = 8; //for 3515
#endif
                break;
            case _AI_TALKBACK_:
                audio_chn_num = 2;
#if defined(_AV_PLATFORM_HI3521_) || defined(_AV_PLATFORM_HI3531_)||defined(_AV_PLATFORM_HI3515_)
                pAi_attr->enWorkmode = AIO_MODE_I2S_MASTER;
#endif
#if defined(_AV_PLATFORM_HI3520D_)||defined(_AV_PLATFORM_HI3520D_V300_)||defined(_AV_PLATFORM_HI3515_)
                audio_chn_num = 16;
#endif
                audio_chn_num = 2; //for 3515
                pAi_attr->u32ClkSel = 0; //! for 3515       
...
#ifdef  _AV_PLATFORM_HI3520D_V300_
    printf("\nai_dev(%d),ai_attr:(%d,%d,%d,%d,%d,%d,%d,%d,%d,%d)\n",ai_dev[1],ai_attr.enSamplerate,ai_attr.enBitwidth,ai_attr.enWorkmode,ai_attr.enSoundmode,
...
//ai_attr:(8000,1,0,0,1,30,320,16,16,0)
#endif
     printf("\nai_dev(%d),ai_attr:(%d,%d,%d,%d,%d,%d,%d,%d,%d)\n",ai_dev[1],ai_attr.enSamplerate,ai_attr.enBitwidth,ai_attr.enWorkmode,ai_attr.enSoundmode,
    		ai_attr.u32EXFlag,ai_attr.u32FrmNum,ai_attr.u32PtNumPerFrm,ai_attr.u32ChnCnt,ai_attr.u32ClkSel); 
    ...
    if((product_type == PTD_D5_35))//||(product_type == PTD_A5_II))
    {
    	    ai_attr.u32ClkSel = 0;
...
int CHiAi::HiAi_start_ai_chn(AUDIO_DEV ai_dev, AI_CHN ai_chn) const
{
    ...
    vqe_Config.bEqOpen = HI_FALSE;

    vqe_Config.s32WorkSampleRate = ai_attr.enSamplerate;
    vqe_Config.stAecCfg.bUsrMode = HI_FALSE;
    vqe_Config.stAecCfg.s8CngMode = AUDIO_AEC_MODE_CLOSE;
    vqe_Config.stAnrCfg.bUsrMode = HI_FALSE;
    vqe_Config.stHpfCfg.enHpfFreq   = AUDIO_HPF_FREQ_150;
#else
...
int CHiAi::HiAi_ai_bind_aenc( ai_type_e eAiType, int phy_chn, int aencChn )
{
    AUDIO_DEV ai_dev = 0;
    AI_CHN ai_chn = 0;
    ...
    if( this->HiAi_get_ai_info( eAiType, phy_chn, &ai_dev, &ai_chn ) != 0 )
    {
        DEBUG_ERROR( "CHiAi::HiAi_ai_bind_aenc HiAi_get_ai_info FAILED aitype=%d phychn=%d venchn=%d\n", eAiType, phy_chn, aencChn );
        return -1;
    }
    HI_S32 sRet = HI_MPI_AENC_BindAi(aencChn, ai_dev, ai_chn);

    if( sRet != HI_SUCCESS )
    {
        DEBUG_ERROR( "CHiAvEncoder::Ae_ai_ae_bind_control HI_MPI_SYS_Bind error sRet=0x%08x type:%d phychn:%d vench=%d ai_dev=%d ai_chn=%d\n", sRet, eAiType, phy_chn, aencChn, ai_dev, ai_chn);
        return -1;
    }
...
int CHiAi::HiAi_ai_unbind_aenc( ai_type_e eAiType, int phy_chn, int aencChn )
{
    AUDIO_DEV ai_dev = 0;
    AI_CHN ai_chn = 0;
    ...
    HI_S32 sRet = HI_MPI_AENC_UnBindAi(aencChn, ai_dev, ai_chn);
